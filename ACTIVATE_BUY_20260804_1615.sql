...
--
-- WHY trading_mode = 'paper' AND A NORMAL ID
-- ------------------------------------------
-- An earlier draft used 'manual' and an id ending in "_manual01". Both were
-- wrong. Every trade the system writes carries trading_mode 'paper'
-- (services/database.py:139), and nothing in the codebase filters on that
-- column -- so 'manual' bought no behaviour and cost consistency: this row
-- would have been the only one with a different value, liable to be split
-- out of any future grouping or to read as "not part of the real results".
--
-- It is part of them. Same map, same targets, same risk, same management.
-- The id follows the generator's exact format
-- (TRADE_%Y%m%d_%H%M%S_%f_<8 hex>, services/database.py:97) so nothing
-- downstream treats it as a special case. How the row was created is
-- recorded where it belongs: in signal_snapshot and activation_reason.
--
-- SAFETY
...
INSERT INTO trades (
    ...
    'TRADE_20260804_161500_445238_a056d0d8',
    'XAU/USD',
    'BUY',
    'BUY',
    'OPEN',
...
-- ============================================================================
-- DELETE FROM trades WHERE id = 'TRADE_20260804_161500_445238_a056d0d8';
