-- =====================================================
-- Gold AI Signals - FULL RESET + clean schema
-- =====================================================
-- ⚠️  WARNING: this DROPS the existing tables (incl. trades) and recreates
--     them cleanly from the canonical schema. Use this when older tables were
--     created with an INCOMPATIBLE structure (e.g. trades.id of type uuid,
--     risk_settings.setting_name, missing columns) that ALTER cannot fix.
--
--     Paper-trading / learning-phase data in these tables WILL BE DELETED.
--     The bot regenerates trades/signals/weights automatically on next runs.
--
-- How to use:
--   Supabase → SQL Editor → New query → paste ALL → Run.
--   After "Success", the trades table will accept TEXT ids like
--   TRADE_20260622_111449_xxxx and the confidence column will exist.
-- =====================================================

-- 1) Drop dependent views first, then tables (CASCADE clears FKs/indexes).
...
-- 2) Recreate everything cleanly from the canonical schema below.
-- =====================================================
...
-- =====================================================
-- Reload PostgREST schema cache so the new columns/types are recognised.
-- =====================================================
