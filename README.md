...
```text
1. cron-job.org يشغّل GitHub Action كل 10 دقائق داخل نافذة التداول 11:00-18:59
...
| المهمة | الجدولة |
|---|---|
| التحليل والإشارات | كل 10 دقائق من 11:00 صباحاً حتى 19:00 مساءً بتوقيتك، أيام العمل فقط، عبر cron-job.org (`workflow_dispatch`) |
...
#### Analysis — كل 10 دقائق أيام العمل داخل وقت التداول فقط

وقت التداول للإشارات: من 11:00 صباحاً حتى قبل 19:00 مساءً بتوقيتك المحلي.
...
> مهم: `send_status=false` يعني يحلل فقط، ولا يرسل Telegram إلا إذا وُجدت صفقة فعلية أو حدث خطأ. كذلك الكود نفسه يمنع توليد الإشارات خارج نافذة 11:00-18:59 حتى لو تم تشغيل Workflow بالخطأ.
