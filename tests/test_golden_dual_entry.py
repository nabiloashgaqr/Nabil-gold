...
def test_close_beyond_0786_invalidates() -> None:
    r = _review(low=4315.0, close=4315.5)
    assert r["action"] == "NONE"
    assert "0.786" in r["reason"]


def test_pending_outside_the_band_keeps_ordinary_governance() -> None:
    r = _review(low=4327.0, close=4330.0, pending=4340.0)
    assert r["action"] == "NONE"
    assert "band" in r["reason"]
...
def test_sell_side_mirrors() -> None:
    ...
    r = review_golden_dual_entry(
        ...
        pending_entry=4352.0,   # inside SELL band 4343.41..4355.21
        qualified_support=2)
    assert r["action"] == "GOLDEN_DUAL_ENTRY"
