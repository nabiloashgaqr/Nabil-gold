...
(`groq_observation_enabled`, `observation_min_conf`, `groq_obs`) that
were only defined inside _ai_decision. The production path
(analyze_async / decide_async) crashed with NameError as soon as Groq
...
def _base_config():
    return {
        ...
        "ai_service": {"enabled": True, "provider": "groq", "fallback_to_classic": False},
    }
...
class TestFinalDecisionAiAvailable:
    ...
    Before the fix, all of these raised:
        NameError: name 'groq_observation_enabled' is not defined
    """

    def test_groq_buy_above_min_confidence_returns_buy(self):
    ...
    def test_groq_sell_above_min_confidence_returns_sell(self):
        agent = DecisionAgent(_base_config())
        ai = {
            "available": True,
            "signal": "SELL",
...
class TestAnalyzeAsyncRegression:
    """analyze_async used to crash with NameError when ai_service returns
    an available response. This test exercises the production path with a
    fake AI that mimics a successful Groq call.
    """
    ...
    async def test_analyze_async_with_available_ai_does_not_raise_name_error(self):
        ...
        agent = DecisionAgent(_base_config(), ai_service=FakeAI())
        results = {
            "all_agents_results": {
